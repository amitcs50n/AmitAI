"""Best-effort locked and explicitly zeroized database-key memory."""
...
class DatabaseKeyHandle:
    ...
    def __init__(self, secret: bytes | bytearray, *, require_lock: bool = True) -> None:
        if len(secret) != 32:
            raise SecureMemoryError("Database key must contain exactly 32 bytes")
        ...
        if require_lock and not self._locked:
            self.close()
            raise SecureMemoryError("Database key memory could not be locked")
    ...
    def _lock_memory(self) -> bool:
        ...
        if sys.platform.startswith("linux"):
            ...
            if madvise is not None:
                ...
                page_end = (
                    (self._address + len(self._buffer) + page_size - 1)
                    // page_size
                ) * page_size
                madvise(
    ...
    def _ensure_open(self) -> None:
        if self._closed:
            raise SecureMemoryError("Database key handle is closed")
    ...
    def __repr__(self) -> str:
        ...
        return f"<DatabaseKeyHandle {state}>"
